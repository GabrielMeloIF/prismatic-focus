...
export default function BottomNav() {
  ...
  return (
    <div
      className={`shadown-sm w-full bg-white py-6 transition-all duration-500 ${
    ...
    >
      <div className='flex w-full items-center justify-between px-[8%] text-gray-700 lg:px-[16%]'>
        ...
        <Link
          href='/'
          className='font-audiowide text-bold text-3xl font-bold lg:hidden'
        >
          Prismatic<span className='text-[var(--second)]'>Focus</span>
        </Link>
            ...
            {navLinks.map((link) =>
              link.dropdown ? (
                  ...
                  <div className='absolute left-0 top-full hidden min-w-[170px] rounded-lg border border-gray-100 bg-white p-2 shadow-xl group-hover:block'>
                    {link.dropdown.map((item) => (
                      <Link
                        key={link.label}
                        href={item.href}
                        className='font-golos block px-4 py-3 font-bold text-[var(--black)] transition-colors hover:bg-[var(--prim-light)] hover:text-[var(--second)]'
                      >
                        {item.label}
                      </Link>
                    ))}
                ...
                <Link
                  key={link.label}
                  href={link.href}
                  className='font-golos flex items-center gap-1 font-bold text-[var(--second)]'
                >
                  {link.label}
                  <Image
                    src='/Menu-dot.svg'
        ...
        <div className='hidden items-center gap-6 lg:flex'>
          <Link
            href='/login'
            className='font-golos border-b border-gray-400 font-semibold text-[var(--black)]'
          >
            Login / Register
          </Link>
        ...
        <div className='flex items-center justify-end gap-4 lg:hidden'>
          ...
          >
            <i className='bi bi-cart3'></i>
            <span className='text-ts absolute -right-2 -top-2 flex h-5 w-5 items-center justify-center rounded-full bg-black text-white'>
              2
            </span>
          </Link>
          <button
        ...
        <div className='mt-3 border-t border-gray-200 bg-white lg:hidden'>
            <nav className='flex flex-col px-[6%] py-4'>
                <Link
                    ...
                    className='font-golos border-b border-gray-100 px-2 py-3 font-semibold text-[var(--black)]'
                >
                    Login / Register
                </Link>
                {navLinks.map((link) =>
                    link.dropdown ? (
                        <div key={link.label} className='border-b border-gray-100 py-1'>
                            <button
                                ...
                                className='font-golos flex w -full items-center justify-between rounded-md px-2 py-3'
                            >
                                {link.label}
                                <i
                                ...
                                <div className='flex flex-col gap-2 bg-[var(--prim-light)] p-2'>
                                    {link.dropdown.map((item) => (
                                        <Link
                                            ...
                                            href={item.href}
                                            onClick={closeMobileMenu}
                                            className='font-golos rounded-md bg-white px3 py-2 text-sm text-[var(--black)] hover:bg-gray-100'
                                        >
                                            {item.label}
                                        </Link>
                                    ))}
